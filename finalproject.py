...
def fbconnect():
    ...
    url =  url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' % (
...
def editCategory(category_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('thank you for Successfully Editing this category %s' % editedcategory.name)
            return redirect(url_for('showCategories'))
    else:
...
def editStudio(category_id, studio_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editstudioitem.html', category_id=category_id, studio_id=studio_id,
            item=editedstudio)
